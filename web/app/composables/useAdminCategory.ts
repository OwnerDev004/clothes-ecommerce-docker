...
export const useAdminCategory = () => {
  ...
  //normalizeData
  const normalizeAdminCategoryData = (
    category: AdminCategoryRecord,
  ): AdminCategoryRecordTable => {
  ...
  const dataTable = computed<AdminCategoryListResponse>(() => {
    return {
      data: categoriesResponse.value?.data
        ? categoriesResponse.value?.data.map(normalizeAdminCategoryData)
        : [],
