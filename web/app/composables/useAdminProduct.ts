...
export type AdminProductVariantRecord = {
  ...
  sku?: string | null
  color?: string | null
  size?: { id?: number | string; name?: string | null; sort_order?: number | null } | null
...
export const useAdminProduct = () => {
  const normalizeAdminProductRow = (product: AdminProductRecord): AdminProductTableRow => {
    const variants = Array.isArray(product.variants) ? product.variants : []
    const firstVariant = variants[0] || {}
    const quantity = variants.reduce((sum, variant) => {
      return sum + Number(variant?.stock_quantity || 0)
    }, 0)
    const image = product.thumbnail?.image_url || product.images?.[0]?.image_url || ''

    return {
      ...product,
      image,
      previewImages: product.images?.length
        ? product.images.map((item) => String(item?.image_url || '')).filter(Boolean)
        : image
          ? [image]
          : [],
