...
export const useProductDetail = () => {
  ...
  const selectedVariant = computed(() => {
    return (
      (product.value?.variants || []).find((variant) => {
        const colorOk = selectedColor.value
          ? variant.color === selectedColor.value
          : true;
        const sizeOk = selectedSizeId.value
  ...
  const selectedVariantPreviewImage = computed(() => {
    return (
      selectedImage.value ||
      imageList.value[0] ||
      "/img/products/default_image.webp"
    );
  });

  const productKey = computed(
    () => `product-detail:${productId.value || "missing"}`,
  );

  const fetchProductById = async (id: string | number) => {
    const response: any = await fetchWithTimeout(`${apiBase}/products/${id}`, {
      method: "GET",
    });
  ...
  const fetchRelatedProducts = async (currentProduct: ProductDetail | null) => {
    try {
      ...
      return rows
        .filter(
          (row: ProductApiRecord) =>
            Number(row?.id) !== Number(currentProduct?.id),
        )
        .slice(0, 4)
  ...
  const fetchDetailSections = async (id: string | number) => {
    try {
      const response: any = await fetchWithTimeout(
        `${apiBase}/products/${id}/detail-sections`,
        {
          method: "GET",
        },
      );
      ...
      return {
        product_detail: Array.isArray(payload?.product_detail)
          ...
          : [],
        faqs_detail: Array.isArray(payload?.faqs_detail)
          ? payload.faqs_detail
          : [],
      };
    } catch {
  ...
  const pageErrorState = computed(
    () => productFetch.error.value?.message || "",
  );
  ...
  const loadProductExtras = async (
    ...
    try {
      const [relatedRowsResult, detailRowsResult, reviewRowsResult] =
  ...
  watch(
    ...
    (currentProduct) => {
      ...
      selectedImage.value =
        previewImages[0] || "/img/products/default_image.webp";
