...
export const normalizeProductCard = (
  item: ProductApiRecord,
  apiBase: string,
): ProductCardItem => {
  const thumbnail =
    item?.thumbnail?.image_url || item?.images?.[0]?.image_url || "";
...
export const normalizeImageList = (
  apiBase: string,
  images: Array<ProductImageRecord | undefined | null>,
) => {
  const rows = images.map((image) => image?.image_url || "").filter(Boolean);

  return Array.from(new Set(rows)).map((path) =>
    resolveApiImageUrl(apiBase, path),
  );
};
