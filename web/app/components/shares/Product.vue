  ...
  <div class="flex items-center gap-16 flex-col desktop:flex-row mb-10">
    <div class="flex gap-14 flex-col-reverse desktop:flex-row items-center">
      ...
      <div class="grid grid-cols-3 desktop:grid-cols-1 gap-3">
        <button v-for="(image, imageIndex) in images" :key="imageIndex"
          class="focus:opacity-60 w-full desktop:w-[152px] h-full desktop:h-[167px] rounded-2lg" @click="
            fullSizeClick(image.fullsize);
          setCurrentIndex(imageIndex);
          " aria-label="View full-size image">
          <NuxtImg class="rounded-md hover:opacity-70 w-full h-full object-cover" :src="image.thumbnail"
      ...
      <div class="w-full desktop:w-[440px] h-full desktop:h-[530px] rounded-3xl">
        <NuxtImg v-if="selectedImage" :src="selectedImage" @click="handleClick"
          class="rounded-3xl w-full h-full object-cover" alt="Selected product image" />
      </div>

      <!-- Lightbox Overlay -->
      <div v-show="isLightbox" @click="handleOverlayClick"
        ...
        <div
          ...
          <!-- Previous Button -->
          <NuxtImg src="/img/products_view/image1_2_thump.png" @click.stop="prevImage"
          ...
          <!-- Next Button -->
          <NuxtImg src="/img/products_view/image1_2_thump.png" @click.stop="nextImage"
          ...
          <!-- Close Button (SVG) -->
          <svg class="fill-white active:fill-orange cursor-pointer absolute top-2 right-2 sm:top-4 sm:right-4"
            width="15" height="25" xmlns="http://www.w3.org/2000/svg" @click="handleClick">
            <path
              d="m11.596.782 2.122 2.122L9.12 7.499l4.597 4.597-2.122 2.122L7 9.62l-4.595 4.597-2.122-2.122L4.878 7.5.282 2.904 2.404.782l4.595 4.596L11.596.782Z" />
          </svg>

          <!-- Lightbox Image -->
          <NuxtImg class="rounded-lg w-full h-auto object-contain mt-4" :src="images[currentIndex]?.fullsize"
    ...
    <div class="w-full desktop:w-1/2">
      <section class="border-b py-4 border-b-black">
        ...
        <SharesDiscount :discount-amount="1" :price="20" :discount-type="2" class="!text-3xl font-extrabold"
      ...
      <section class="border-b py-4 border-b-black">
        ...
        <div class="flex gap-4">
          <button v-for="(color, index) in colors" :key="index" :style="{ backgroundColor: color.hex }"
            @click="selectColor(color)" :aria-label="`Select color ${color.name}`"
            class="w-10 h-10 rounded-full border border-gray-300 cursor-pointer hover:opacity-80 relative">
            <span v-if="selectedColor?.name === color.name"
      ...
      <section class="border-b py-4 border-b-black">
        ...
        <div class="flex overflow-auto gap-2 hide-scrollbar">
          <div class="grid grid-flow-col gap-4 overflow-x-visible whitespace-nowrap">
            <button v-for="(size, index) in sizes" :key="index" @click="selectSize(size)" :class="[
