...
const dismissedTelegramBanner = useCookie('dismissed_telegram_banner', { default: () => '0' })
const connectingTelegram = ref(false)

const telegramProfile = computed(() => {
  const p = authStore.userProfile || {}
  return { chatId: p.telegram_chat_id, userId: p.telegram_user_id }
})

const isTelegramConnected = computed(() => Boolean(telegramProfile.value.chatId || telegramProfile.value.userId))

const dismissBanner = () => {
  showTelegramBanner.value = false
  dismissedTelegramBanner.value = '1'
}

const connectTelegramBanner = async () => {
...
watchEffect(() => {
  ...
  if (dismissedTelegramBanner.value === '1') {
    showTelegramBanner.value = false
    return
  }
...
<template>
  <main>
    <Teleport v-if="showTelegramBanner" to="body">
      <div
        class="fixed inset-x-0 top-0 z-50 flex items-center justify-between gap-3 bg-gradient-to-r from-[#0088CC] to-[#00A3E0] px-5 py-2.5 text-white shadow-md">
        <div class="flex items-center gap-2 text-sm">
          <Icon name="fa-brands:telegram-plane" class="text-lg" />
          <span>Stay updated! Get order notifications on Telegram.</span>
        </div>
        <div class="flex items-center gap-2 shrink-0">
          <el-button size="small" :loading="connectingTelegram"
            class="!text-[#0088CC] !bg-white !border-white hover:!bg-gray-100" @click="connectTelegramBanner">
            Connect
          </el-button>
          <button class="text-white/70 hover:text-white transition-colors" @click="dismissBanner">
    ...
    <section class="px-5 desktop:container pt-6">
      <div class="hero-carousel-wrapper rounded-[32px] border border-zinc-200 overflow-hidden">
        <el-carousel interval="5500" arrow="hover" height="560px" autoplay pause-on-hover>
            ...
            <div class="grid min-h-[560px] items-center gap-8 px-6 py-8 lg:grid-cols-[1.05fr_0.95fr] lg:px-10 xl:px-14"
              ...
              <div class="max-w-2xl">
                <p v-if="slide.subtitle"
                  class="inline-flex rounded-full border border-zinc-300 bg-white/80 backdrop-blur-sm px-4 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-600 shadow-sm">
                  {{ slide.subtitle }}
                </p>
                <h1
                  class="mt-5 font-Poppins text-5xl font-extrabold leading-[1.02] text-zinc-950 md:text-6xl xl:text-7xl">
                  {{ slide.title }}
                </h1>
              ...
              <div class="relative flex justify-center lg:justify-end">
                <div
                  class="absolute inset-0 rounded-[28px] bg-[radial-gradient(circle_at_center,rgba(0,0,0,0.06),transparent_60%)]" />
                <template v-if="slide.image_url">
                  <img :src="slide.image_url" :alt="slide.title" width="669" height="663"
                    :loading="index === 0 ? 'eager' : 'lazy'" :fetchpriority="index === 0 ? 'high' : 'auto'"
                    decoding="async" class="relative z-10 w-full max-w-[620px] object-contain" />
                </template>
                <div v-else
                  class="relative z-10 flex w-full max-w-[400px] items-center justify-center rounded-2xl bg-white/40 py-20">
                  <div class="text-center">
                    <Icon name="mdi:image-off-outline" class="text-5xl text-zinc-300" />
    ...
    <section class="px-5 desktop:container pt-10">
      <div
        class="grid grid-cols-2 divide-x divide-zinc-100 overflow-hidden rounded-2xl border border-zinc-100 bg-white md:grid-cols-4">
        <div
          class="flex flex-col items-center gap-2 px-4 py-6 text-center transition-colors duration-200 hover:bg-zinc-50 sm:px-6 sm:py-7">
        ...
        <ClientOnly>
          <Swiper :modules="[SwiperAutoplay]" :slides-per-view="2" :space-between="8" :breakpoints="{
            ...
            <SwiperSlide v-for="slide in brands" :key="slide.id">
              <NuxtLink :to="getBrandRoute(slide.id)"
                class="block rounded-xl bg-gray-200 h-[96px] md:h-[110px] transition-all duration-200 hover:scale-[1.02] hover:shadow-lg">
    ...
    <section class="section-animate px-5 desktop:container py-12 transition-all duration-700 delay-100">
      ...
      <div class="mt-8 text-center">
        ...
        <div v-if="isLoadingProducts && products.length"
          class="flex items-center justify-center gap-2 text-sm text-zinc-500">
    ...
    <section class="section-animate px-5 desktop:container pb-12 transition-all duration-700 delay-300">
      <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-8">
        <div>
          <div class="flex items-center gap-2">
            ...
            <span
              class="inline-flex items-center gap-1 rounded-full bg-red-50 px-3 py-0.5 text-xs font-semibold text-red-500">
      ...
      <div v-else-if="topSellingProducts.length"
        class="grid gap-4 sm:grid-cols-2 tablet:grid-cols-3 desktop:grid-cols-4 xl:gap-6">
        <div v-for="(item) in topSellingProducts" :key="`top-${item.id}`" class="relative">
          <!-- Badge -->
          <div
            class="absolute -top-1.5 -left-1.5 z-10 flex items-center gap-1 rounded-full bg-red-500 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white shadow-lg">
      ...
      <div class="bg-white rounded-3xl border border-zinc-200 overflow-hidden">
        ...
        <div class="grid gap-5 p-6 md:grid-cols-3 md:p-8 md:gap-6">
          <div v-for="(style, index) in collectionItems" :key="style.id"
            ...
            <NuxtImg :src="resolveVisualImage(style.image_url, `/img/dress_styles/style${(index % 4) + 1}.png`)"
              :alt="style.name || 'Collection'"
              class="h-full w-full object-cover transition-all duration-700 group-hover:scale-110" format="webp"
              densities="x1" loading="lazy" />
            <!-- Dark gradient overlay -->
            <div
              class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent transition-all duration-500 group-hover:from-black/90" />
            <!-- Content -->
            <div class="absolute bottom-0 left-0 right-0 p-6 md:p-8">
              <h3
                class="text-xl font-bold text-white md:text-2xl transition-all duration-300 group-hover:translate-y-[-4px]">
                {{ style.name }}
              </h3>
              ...
              <div
                class="mt-3 translate-y-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
                <span
                  class="inline-flex items-center gap-1.5 rounded-full bg-white px-4 py-1.5 text-xs font-semibold text-zinc-900 shadow-lg hover:bg-zinc-100 transition-colors">
                  Shop Now
    ...
    <section class="section-animate px-5 desktop:container pb-12 transition-all duration-700 delay-500">
      ...
      <div v-else-if="customers_review.length !== 0">
        <ClientOnly>
          <Swiper :modules="[SwiperAutoplay]" :slides-per-view="1" :space-between="20" :breakpoints="{
            640: { slidesPerView: 1.5, spaceBetween: 20 },
            1024: { slidesPerView: 3, spaceBetween: 24 },
            1440: { slidesPerView: 4, spaceBetween: 24 },
          }" :autoplay="{ delay: 4000, disableOnInteraction: true, pauseOnMouseEnter: true }" class="pb-2">
            <SwiperSlide v-for="(slide) in customers_review" :key="slide.id">
              <div
                class="flex h-full flex-col rounded-2xl border border-zinc-100 bg-white p-6 shadow-sm transition-all duration-200 hover:shadow-md hover:-translate-y-0.5">
                <!-- Star Rating -->
                <div class="flex items-center gap-0.5">
                  <Icon v-for="star in 5" :key="star" :name="star <= slide.rating ? 'mdi:star' : 'mdi:star-outline'"
                    class="text-base" :class="star <= slide.rating ? 'text-amber-400' : 'text-zinc-200'" />
                </div>
                <!-- Customer Info -->
                <div class="mt-3 flex items-center gap-2.5">
                  <div
                    class="flex h-10 w-10 items-center justify-center rounded-full bg-zinc-800 text-xs font-bold text-white uppercase">
