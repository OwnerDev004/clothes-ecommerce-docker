...
export default defineEventHandler((event) => {
  ...
  const dt = config.pdfmakeDevtools ?? {};
  const embedded = dt.embedded ?? [];
  const cdn = dt.cdn ?? [];
  const googleFonts = dt.googleFonts ?? [];
  const fontSizesKB = dt.fontSizesKB ?? {};
  const embeddedKB = dt.embeddedKB ?? dt.estimatedKB ?? 0;
  const useDefaultRoboto = dt.useDefaultRoboto ?? true;
  const apiEnabled = dt.apiEnabled ?? false;
  const googleFontSet = new Set(googleFonts);
  const fallbackEmbeddedKB = Math.round(embeddedKB / Math.max(embedded.length, 1));
  const fontRows = [
    ...embedded.map(
      (f) => `<tr><td>${f}</td><td><span class="badge ${googleFontSet.has(f) ? "google" : "embedded"}">${googleFontSet.has(f) ? "Google" : "Embedded"}</span></td><td>${fontSizesKB[f] ?? fallbackEmbeddedKB} KB</td></tr>`
    ),
    ...cdn.map(
      (f) => `<tr><td>${f}</td><td><span class="badge cdn">CDN</span></td><td>\u2014</td></tr>`
    ),
    ...googleFonts.filter((f) => !embedded.includes(f)).map(
      (f) => `<tr><td>${f}</td><td><span class="badge google">Google</span></td><td>\u2014</td></tr>`
    ),
    useDefaultRoboto ? `<tr><td>Roboto</td><td><span class="badge cdn">Default CDN</span></td><td>\u2014</td></tr>` : ""
  ].filter(Boolean).join("\n");
  const noFonts = embedded.length === 0 && cdn.length === 0 && googleFonts.length === 0 && !useDefaultRoboto;
...
<body>
  <h1>\u{1F5D2} PDFMake</h1>
  <p class="meta">nuxt-pdfmake v1.0.0 \u2014 Font configuration &amp; PDF generator</p>

  <div class="section">
    <h2>Registered Fonts</h2>
    ${noFonts ? `<p class="empty">No fonts configured. Add <code>fonts</code> to your pdfmake module options.</p>` : `<table>
  ...
  <div class="section">
    ...
    ${!apiEnabled ? `<p class="warning">\u26A0 The <code>/_pdfmake/generate</code> API route is disabled. Set <code>enableApiRoute: true</code> in module options to enable it.</p>` : `<label for="dd">Document definition (JSON)</label>
        <textarea id="dd">{
  "content": [
