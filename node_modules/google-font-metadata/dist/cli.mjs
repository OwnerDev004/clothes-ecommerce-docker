...
import { f as fetchAPI, o as fetchVariable, p as parsev1, a as parsev2, g as generateAxis, q as parseVariable, m as parseIcons, n as parseLicenses, v as validateCLI } from './variable-gen-CvpgmYQ5.mjs';
...
const cli = cac("gfm");
cli.command("generate [key]", "Fetch parsing metadata for all fonts").option("-n, --normal", "Fetch only normal Google Fonts Developer API").option("-v, --variable", "Fetch only variable metadata").action(async (key, options) => {
  try {
...
cli.command("parse [key]", "Process metadata for v1 and v2 from gfm generate").option("-1, --v1", "Only parse v1 metadata").option("-2, --v2", "Only parse v2 metadata").option("-r, --axis-registry", "Only parse axis registry metadata").option("-v, --variable", "Only parse variable metadata").option("-i, --icon", "Only parse icon metadata").option("-l, --license", "Only parse license metadata").option("-f, --force", "Skip cache and force parse all metadata").option("--no-validate", "Skip validating metadata result with schema").action(async (key, options) => {
  try {
    ...
    if (!options.v1 && !options.v2 && !options.variable && !options.icon && !options.license && !options.axisRegistry) {
      if (options.force) {
        consola.info(
...
cli.command("validate", "Validate stored metadata with schema.").option("-1, --v1", "Only validate APIv1 metadata").option("-2, --v2", "Only validate APIv2 metadata").option("--variable", "Only validate variable metadata").action((options) => {
  try {
